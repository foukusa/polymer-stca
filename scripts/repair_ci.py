...
"""Repair STCA 1.0's stale merged-document manifest; dry-run by default.

Run from the supplied, separately staged patch. No network/Git writes, model
training or data collection. Only three engineering files and their manifest
are replaced. Existing public documents and scientific files are never written.
"""
...
CHANGED = ('scripts/apply_update.py', 'scripts/repair_ci.py',
           'tests/test_updated_checkout.py')
...
def updater_module():
    spec = importlib.util.spec_from_file_location('_stca_ci_patch', SOURCE / 'scripts/apply_update.py')
...
def main(argv=None):
    ...
    print('STCA 1.0: merged-document manifest repair')
    ...
    backup = repo.parent / ('STCA_CI_backup_' + datetime.now().strftime('%Y%m%d_%H%M%S_%f'))
    backup.mkdir()
    for name, old, new in plan:
        if old is not None:
            path = backup / name
